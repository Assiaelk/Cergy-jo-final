...
void saveTrainings(char *nameFile, struct Training *trainings, int nbTrainings) {
...
int chargerEntrainements(char *nameFile, struct Training *trainings) {
    ...
    while (fscanf(file, "%d/%d/%d %f %d",
        ...
        if (trainings[count].type == RELAIS_QUATRE_CENTS) {
            for (int i = 0; i < 4; i++) {
                fscanf(fichier, "%s %d", trainings[count].participants[i].name, &trainings[count].participants[i].position);
            }
...
void consultStatistics(char *nameAthlete, struct Training *trainings, int nbTrainings, enum Tests test) {
    ...
    for (int i = 0; i < nbTrainings; i++) {
        if (traings[i].type == test && strcmp(trainings[i].participants[0].name, nameAthlete) == 0) {
    ...
    while (!quit) {
        ...
        switch (choice) {
            case 1:
                {
                    ...
                    inputTraining(&newTraining, nameAthlete, trainings, &nbTrainings);
                }
                break;
            case 2:
                printf("Nom du fichier à charger: ");
                fgets(nomFichier, MAX_NAME_LENGHT, stdin);
                nameFile[strcspn(nameFile, "\n")] = '\0'; // Remove trailing newline
                nbTrainings = chargeTrainings(nameFile, trainings);
                break;
            case 3:
                printf("Nom du fichier à sauvegarder: ");
                fgets(nameFile, MAX_NAME_LENGHT, stdin);
                nameFile[strcspn(nameFile, "\n")] = '\0'; // Remove trailing newline
                saveTrainings(nameFile,trainings, nbTrainings);
                break;
            case 4:
                consultHistoric(trainings,nbTrainings);
                break;
            case 5:
                printf("Nom de l'athlète: ");
                fgets(nameAthlete, MAX_NAME_LENGHT, stdin);
                nameAthlete[strcspn(nameAthlete, "\n")] = '\0'; // Remove trailing newline
                consultStatistics(nameAthlete, trainings, nbTrainings, test);
                break;
            case 6:
                printf("Nom de l'athlète: ");
                fgets(nameAthlete, MAX_NAME_LENGHT, stdin);
                nameAthlete[strcspn(nameAthlete, "\n")] = '\0'; // Remove trailing newline
                resumePerformancesAthlete(nameAthlete, trainings, nbTrainings, test);
                break;
            case 7:
                printf("Nom de l'athlète: ");
                fgets(nameAthlete, MAX_NAME_LENGHT, stdin);
                nameAthlete[strcspn(nameAthlete, "\n")] = '\0'; // Remove trailing newline
                progressAthlete(nameAthlete, trainings, nbTrainings, test);
                break;
            case 8:
                printf("Choisissez le type d'épreuve :\n");
                printf("0. 100 mètres\n");
                ...
                findBestAthletes(trainings, nbTrainings, test);
                break;
            case 9:
                {
                    struct Date date2;
                    ...
                    if (scanf("%d/%d/%d", &date.day, &date.month, &date.year) == 3) {
                        ...
                        if (scanf("%d/%d/%d", &date2.day, &date2.month, &date2.year) == 3) {
                            differenceTime(trainings, nbTrainings, nameAthlete, test, date, date2);
                        } else {
                            printf("Format de date invalide.\n");
                            while (getchar() != '\n');
                        }
            ...
            case 10:
                ...
                fgets(nameAthlete, MAX_NAME_LENGHT, stdin);
                nameAthlete[strcspn(nameAthlete, "\n")] = '\0'; // Remove trailing newline
                regressLinear(nameAthlete, trainings, nbTrainings, test);
                break;
            case 11:
                quit = true;
                break;
